        ...
        if i==0: # 첫 번째 프레임이라면?
          ...
          if result != []: #감지된 얼굴이 존재한다면
            for person in result:
              ...
              cv2.putText(frame, "id: "+str(id_), (int(x),int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2, cv2.LINE_AA)
              id_+=1

          # tracker 초기 세팅 [tracker,추적중인 객체 id]  
          for t in range(len(tem)):
            trackers.append([cv2.TrackerCSRT_create(),tem[t][4]]) # trackers에 [트래커, id]
            trackers[t][0].init(frame,(tem[t][0],tem[t][1],tem[t][2],tem[t][3])) # [x,y,w,h]로 탐지한 각 얼굴마다 tracker 초기화
